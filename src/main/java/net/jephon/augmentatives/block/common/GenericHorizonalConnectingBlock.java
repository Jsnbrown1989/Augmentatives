...
public abstract class GenericHorizonalConnectingBlock
        extends Block {
    ...
    protected GenericHorizonalConnectingBlock(float radius1, float radius2, float boundingHeight1, float boundingHeight2, float collisionHeight, AbstractBlock.Settings settings) {
        super(settings);
    ...
    protected abstract MapCodec<? extends GenericHorizonalConnectingBlock> getCodec();
    ...
    protected int getShapeIndex(BlockState state) {
        return this.SHAPE_INDEX_CACHE.computeIntIfAbsent(state, statex -> {
            ...
            if (statex.get(NORTH).booleanValue()) {
                i |= GenericHorizonalConnectingBlock.getDirectionMask(Direction.NORTH);
            }
            if (statex.get(EAST).booleanValue()) {
                i |= GenericHorizonalConnectingBlock.getDirectionMask(Direction.EAST);
            }
            if (statex.get(SOUTH).booleanValue()) {
                i |= GenericHorizonalConnectingBlock.getDirectionMask(Direction.SOUTH);
            }
            if (statex.get(WEST).booleanValue()) {
                i |= GenericHorizonalConnectingBlock.getDirectionMask(Direction.WEST);
            }
            return i;
        });
    }
