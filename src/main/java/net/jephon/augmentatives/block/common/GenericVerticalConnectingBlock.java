...
public abstract class GenericVerticalConnectingBlock
        extends Block {
    ...
    protected GenericVerticalConnectingBlock(float radius1, float radius2, float boundingHeight1, float boundingHeight2, float collisionHeight, Settings settings) {
        super(settings);
    ...
    protected abstract MapCodec<? extends GenericVerticalConnectingBlock> getCodec();
    ...
    protected int getShapeIndex(BlockState state) {
        return this.SHAPE_INDEX_CACHE.computeIntIfAbsent(state, statex -> {
            ...
            if (statex.get(UP).booleanValue()) {
                i |= GenericVerticalConnectingBlock.getDirectionMask(Direction.UP);
            }
            if (statex.get(DOWN).booleanValue()) {
                i |= GenericVerticalConnectingBlock.getDirectionMask(Direction.DOWN);
            }
            return i;
        });
    }
