...
import static net.jephon.augmentatives.util.Colorizer.PINK;


public class SpeedLimitSignBlock
        extends Block {
    public static final DirectionProperty FACING = HorizontalFacingBlock.FACING;
    public static final EnumProperty<MultiBlockUtil.MultiblockPart> PART = MultiBlockProperties.MULTIBLOCK_PART;
    public static final EnumProperty<SpeedLimitSpeeds> SPEED= EnumProperty.of("speed", SpeedLimitSpeeds.class);
    ...
    public SpeedLimitSignBlock(Settings settings) {
        super(settings);
        this.setDefaultState((BlockState)((BlockState)this.stateManager.getDefaultState()).with(FACING, Direction.NORTH).with(PART, MultiBlockUtil.MultiblockPart.HEAD).with(SPEED, SpeedLimitSpeeds.fifteen));
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        world.setBlockState(pos, (BlockState)state.cycle(SPEED), Block.NOTIFY_ALL);
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!world.isClient) {
            if (player.isCreative()) {
                SpeedLimitSignBlock.onBreakInCreative(world, pos, state, player);
            } else {
                BlockPos blockPos;
    ...
    private void placeAdditionalBlock(World world, BlockPos pos, BlockPos originPos, Direction direction, BlockState state, MultiBlockUtil.MultiblockPart part) {
        world.setBlockState(pos, ModBlocks.OUTSIDE_SIGN_SPEEDLIMIT.getDefaultState().with(PART, part).with(FACING, direction));
